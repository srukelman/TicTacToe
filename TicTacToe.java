...
public class TicTacToe extends JFrame implements ActionListener {
  ...
  private JButton oButton, xButton;//color change buttons
  private JButton onePlayer, twoPlayers;//numPlayerz buttons
  private Board board;//board
  private int lineThickness=4;//starting line thickness
  private Color oColor=Color.BLUE, xColor=Color.RED;//starting colors
  private char whoseTurn = 'X';//whose turn is it
  private boolean comp = false;
  static final char BLANK=' ', O='O', X='X';//char variable for each cell on the grid
  private char position[]={  // Board position (BLANK, O, or X)
    BLANK, BLANK, BLANK,
    BLANK, BLANK, BLANK,
    BLANK, BLANK, BLANK};
  private int xwins=0, owins=0, draws=0;  // game count
  ...
  // Initialize
  public TicTacToe() {
    super("Tic Tac Toe");//call the super constructor to create a GUI with title
    JPanel topPanel=new JPanel();//create top panel
    topPanel.setLayout(new FlowLayout());//set layout
    topPanel.add(onePlayer = new JButton("1 Player"));//add button to set 1 player
    topPanel.add(twoPlayers = new JButton("2 Players"));//add button to set 2 players
    topPanel.add(oButton=new JButton("O Color"));//add button to change color of O
    topPanel.add(xButton=new JButton("X Color"));//add button to change color of X
    oButton.addActionListener(this);//add event listener for O button 
    xButton.addActionListener(this);//add event listener for X button
    onePlayer.addActionListener(this);//add event listener
    twoPlayers.addActionListener(this);//add event listener
    add(topPanel, BorderLayout.NORTH);//add topPanel to GUI
    add(board=new Board(), BorderLayout.CENTER);//create and add new board to GUI
  ...
  private class Board extends JPanel implements MouseListener {
      ...
      if (pos>=0 && pos<9 && position[pos]==BLANK) {
        ...
        if(comp) {                                                                   
        	...
        	if(won(whoseTurn)) {
            	...
            	whoseTurn = (whoseTurn == 'X')?'O':'X';  //change turn
            }
        }
        repaint();
      }
    ...
    //Check for 1 player or 2 players
    void numPlayerz() {
    	String [] options = {"1 Player", "2 Players"};
    	int numPlayaz = JOptionPane.showOptionDialog(null, "How many players?", "Number of Players", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
    	comp = (numPlayaz == 0);  
    }
    // Computer plays X
    void putX() {
    ...
    void newGame(char winner) {
      ...
      if (JOptionPane.showConfirmDialog(null, 
          "X has "+xwins+ " wins, O has "+owins+" wins, there have been "+draws+" draws\n"
      ...
      // Computer starts first every other game if computer is in use
      if ((xwins+owins+draws)%2 == 1 && comp) {
        nextMove();
    }else {
    	whoseTurn = (winner == 'X')?'O':'X';
    }
    }
